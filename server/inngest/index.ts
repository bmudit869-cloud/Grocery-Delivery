...
export const inngest = new Inngest({ id: "groceries-deliveries" });
...
// Monthly offers email on the 1 of every month
const sendMonthlyOffes = inngest.createFunction({
id:"send-monthly-offers",
name:"Monthly Payday Offes",
triggers:[cron("0 10 1 * *")]
    ...
    const {deals,users} = await step.run("fetch-deals-and-users", async()=> {
     ...
     //Send in batches of 10 to avoid overwheming mail server
     const batchSize = 10;
...
const autoAssignRider = inngest.createFunction({
    ...
    const result = await step.run("assign-rider",async()=>{
        ...
        //Skip if order doesn't exist. assigned or already cancelled 
        ...
        await prisma.order.update ({
            where :{id:orderId},
            data: { deliveryPartnerId: availableRider.id, deliveryOtp :otp, status:"Assigned", statusHistory : history}
        })
...
export const functions = [checkLowStock, sendMonthlyOffes, autoAssignRider];