...
const MyOrders = () => {
     ...
     setLoading(true);
     let data = dummyAdminDashboardData.recentOrders || [];
     if (activeTab !== "all") {
     data = data.filter(
  ...
  return (
    <div className="min-h-screen bg-app-cream mb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h2 className="text-2xl font-semibold text-app-green mb-6"> My Orders </h2>
            ...
            {loading ? (
                <Loading/> ): orders.length===0 ? (
                    ...
                    <p className="text-sm text-app-text-light mb-2"> Start shopping to see your orders here </p>
                        ...
                        {orders.map((order)=> (
                            ...
                            <div className="flex items-start justify-between mb-3"> 
                                ...
                                <div> 
                                    ...
                                    {new Date(order.createdAt).toLocaleDateString("en-US",{month:"short", day:"numeric", year:"numeric"})} </span>
                                </div>
                                </div>
                           ...
                           {(order.items?.length || 0) > 4 &&  <div className="size-12 sm:size-16 rounded-lg bg-app-cream flex-center text-xs font-semibold text-app-text-light"> +{(order.items?.length || 0) - 4} </div>
                            } </div>
