...
<main className="flex-1">
     ...
     {loading? (
        ...
        <div className="text-center py-16">
             ...
             <button onClick= {clearFilters}className="px-4 py-2 text-sm font-medium bg-app-green text-white rounded-xl hover:bg-app-green-light transition-colors"> CLear Filters </button>
             </div>
    ): (
    ...
    {totalPages>1 && (
        <div className="flex-center gap-2 mt-16">
            {Array.from({length:totalPages}).map((_, i)=> (
                ...
                className={ `size-9 rounded-lg text-sm font-semibold transition-colors ${page===i+1 ? "bg-app-green text-white" : "bg-white text-app-text-light hover:bg-app-cream"}`}> {i+1} </button>
            ))} 
        </div>
    )}
     </main>
