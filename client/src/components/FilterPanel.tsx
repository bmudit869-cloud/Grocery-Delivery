...
const FilterPanel = ({categories,category, minPrice, maxPrice, updateFilter, hasFilters, clearFilters }: any ) => {
  ...
  return (
    ...
    <div> 
        ...
        {/* Price Range*/}
        <div><h3 className="text-sm font-semibold text-app-green mb-3"> Price Range </h3>
        <div className="flex items-center gap-2">
            <input type="number" 
            placeholder="Min"
            value={minPrice}
            onChange={(e)=> updateFilter('minPrice',e.target.value)} 
            className="w-full px-3 py-2 text-sm bg-white rounded-lg border focus:border-app-border"/>
            ...
            {hasFilters && (
                <button onClick={clearFilters}
                 className="w-full py-2 text-sm text-app-error hover:bg-red-50 rounded-lg transition-colors font-medium"> Clear All Filtes </button>)}
        </div>
  )
}
